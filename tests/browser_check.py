...
try:
    ...
    with sync_playwright() as p:
        ...
        page.goto(BASE, wait_until='networkidle')
        assert not page.locator('#email-inquiry').evaluate('(el) => el.open')
        page.locator('[data-interest="Diamonds & rings"]').first.click()
        assert page.locator('#email-inquiry').evaluate('(el) => el.open')
        assert page.locator('#interest').evaluate('(el) => el === document.activeElement')
        ...
        assert not page.locator('#form-result').is_visible()
        assert 'Details changed' in page.locator('#form-status').inner_text()
        page.locator('#inquiry-form button[type="submit"]').click()
        assert page.locator('#form-result').is_visible()
        assert 'You can reach me at' not in page.locator('#email-draft').input_value()
        assert page.locator('#email-link').evaluate('(el) => el === document.activeElement')
        results.append('Inquiry disclosure, automatic opening, validation, optional email, draft invalidation, encoded mailto, and copy/fallback work')
        page.set_viewport_size({'width': 390, 'height': 844})
        page.locator('#name').focus()
        assert not page.locator('.mobile-bar').is_visible()
        page.locator('#email-inquiry summary').focus()
        assert page.locator('.mobile-bar').is_visible()
        page.goto(BASE, wait_until='networkidle')
        ...
        assert page.locator('#services').evaluate('(el) => el === document.activeElement')
        assert page.locator('#services').bounding_box()['y'] >= page.locator('.header-shell').bounding_box()['height']
        page.goto(BASE + '#email-inquiry', wait_until='networkidle')
        assert page.locator('#email-inquiry').evaluate('(el) => el.open')
        page.set_viewport_size({'width': 1440, 'height': 1000})
        results.append('Keyboard skip/navigation focus, sticky-header clearance, inquiry deep link and unobscured mobile form')
